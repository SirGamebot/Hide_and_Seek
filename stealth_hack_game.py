...
class NPCGuard(pygame.sprite.Sprite):
    ...
    def update(self, player, alarm, emp, walls, panic):
        ...
        if self.state == "patrol":
            ...
            self._move(self.look, self.CHASE_SP, walls)
            if not line_of_sight(self.rect.center, player.rect.center, walls):
                ...
                if self.path and self.idx < len(self.path):
                    waypoint_grid = self.path[self.idx]
                    waypoint_world = pygame.math.Vector2(waypoint_grid[0] * TILE + TILE / 2, waypoint_grid[1] * TILE + TILE / 2)
                    move_vec = waypoint_world - pygame.math.Vector2(self.rect.center)
                    if move_vec.length() < 5:
                        self.idx += 1
                    elif move_vec.length() > 0:
                        self._move(move_vec.normalize(), self.CHASE_SP, walls)
                elif to_pl.length() > 0:
                    self._move(to_pl.normalize(), self.CHASE_SP, walls)
            elif to_pl.length() > 0:
                self._move(to_pl.normalize(), self.CHASE_SP, walls)
        return alarm
